...
class ContactsTableViewController: UITableViewController {
  ...
  func loadUsers() {
    ...
    reference(.User).order(by: kLASTNAME, descending: false).getDocuments { (snapshot, error) in
  ...
  override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if !isGroup {
      ...
      if !checkBlockedStatus(withUser: userToChat) {
        ...
        ProgressHUD.showError("This user is not available for chats!")
      }
    }else {
      //Group
      ...
      self.navigationItem.rightBarButtonItem?.isEnabled = membersOfGroupChat.count > 0
    }
  }
...
extension ContactsTableViewController: UISearchResultsUpdating {
  ...
  func filteredContentForSearchText(searchText: String, scopy: String = "All") {
...
extension ContactsTableViewController: UserTableViewCellDelegate {
